...
2. Create a `.env` file in the project root (SvelteKit loads private env vars from here). At minimum you need:
   ...
   |----------|----------|-------------|
   | `GOOGLE_PROJECT_ID` | Yes | GCP project ID (also read at build time in `src/lib/agent.ts`). |
   | `GOOGLE_LOCATION` or `GOOGLE_LOCATION_REGION` | No | Vertex region; empty or `global` becomes `us-central1`. |
...
- `src/lib/tools.ts` — Builds tool instances from `$env/dynamic/private` via `createVercelAiAgentTools`.
